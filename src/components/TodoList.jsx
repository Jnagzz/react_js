...
const TodoList = () => {
    const [todos, setTodos] = useState([
        ...
        { id: 3, title: "Học Vuex", completed: false }, // todo
    ]);
    ...
    return (
        <div className="tw-max-w-xl tw-mx-auto tw-my-10 tw-border-2 tw-border-red-500 tw-p-5">
            <form className="input-group" onSubmit={onHanleSubmit}>
                <input type="text" className="form-control" placeholder="Learning Javascript" onInput={(e) => setInputValue(e.target.value)} />
            ...
            <ul className="list-group tw-mt-2">
                {todos.map((todo) => (
                    <li key={todo.id} className="list-group-item d-flex align-items-center">
                        {editingTodo === todo.id ? (
                            <>
                                <input type="text" className="form-control input-sm" value={editingText}
                                    onChange={(e) => setEditingText(e.target.value)}
                                />
                                <div className="tw-flex tw-items-center tw-space-x-2 tw-ml-2">
                                    <button className="btn btn-sm btn-info">Hủy</button>
                                    <button className="btn btn-sm btn-primary" onClick={() => saveEditing(todo.id)}>
                                        Lưu
                                    </button>
                                </div>
                            </>
                            ...
                            <>
                                <input className="form-check-input me-3" type="checkbox" checked={todo.completed}
                                    onChange={() => completedTodo(todo.id)} />
                                <label className={`form-check-label ${todo.completed ? "tw-line-through" : ""}`}
                                    onClick={() => startEditing(todo)}>
                                    {todo.title}
                                </label>
                                <button className="btn btn-sm btn-danger ms-auto" onClick={() => removeTodo(todo.id)}>
                                    Xóa
                                </button>
                            </>
                        )}
